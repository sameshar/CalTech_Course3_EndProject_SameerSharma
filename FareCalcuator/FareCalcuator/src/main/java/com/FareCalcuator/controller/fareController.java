...
public class fareController {
	...
	private double randomNum1() {
		...
		if (randomNum1==0) {
			...
			formattedValue = Double.parseDouble(df.format(randomRawNum1));
	...
	private double randomNum2() {
		...
		if (randomNum2==0) {
			...
			DecimalFormat df1 = new DecimalFormat("#.##");
			formattedValue = Double.parseDouble(df.format(randomRawNum2));
	...
	public fareBean fareCalculation(@PathVariable String source, @PathVariable String destination) {
		...
		double destMiles = randomNum1();
		double distance = sourceMiles + destMiles;
